...
Phase 5 将执行策略分为三个显式模式：

- `normal`：本地普通开发使用 changed-only 和 verification cache，不运行 benchmark；
- `ci`：合并和发布使用 full verify，质量失败硬阻断；
- `optimize`：定时任务使用完整 verify；随后由 `cc-optimize` 执行 benchmark
  和趋势分析，只生成候选 change。

`cc-verify` 不带 `--execution-mode` 时保持历史 full verify 兼容行为。使用
`--execution-mode normal` 才会显式选择快速开发路径。`cc-optimize` 只读本地
脱敏事件和 baseline/candidate 记录，输出 `observe`、`propose` 或 `reject`，
不会直接修改 Harness 策略、readset 或业务代码。
...
| `standard` | 核心 + 条件 | 启用 | 完整 | 团队项目、生产代码 |
| `strict` | 全部始终加载 | 启用 + 额外校验 | 双轮完整 | 合规、金融、安全敏感 |
| `loop`（安装默认） | 核心 + 条件 | 启用 + 自动续行 | 完整 + Loop 审计 | 在信任包络内的自动化执行 |
