...
cc-apply 启动 (执行侧, resident 主流程)
  ...
  ③ save pre-wave baseline  → baseline/pre-wave-N.json
...
触发:`wave_plan_generated_and_valid` 之后、首个代码编辑之前。展示:
```text
...
for wave in wave_plan.waves:
    ...
    ④ run_per_task_validation_and_delta:
         for task in wave.tasks:
           ...
           cc-delta-check --before pre-wave-{N} --after 当前
...
| abort | 已 commit 的波保留(不回滚),change 标 blocked,记 cc-event-write |
...
1. 调度器单测(`tests/test_wave_plan.py`):分层 Kahn 正确性 / E_WAVE001 / E_WAVE002 / parallel_safe:false 独占波 / 与 cc-deps.parse_task_files 复用等价性。
...
4. 行为用例(`evals/behavior/` 增):wave 编排含环 → 退出码 1 / 写范围相交 → 退出码 1 / wave-confirmation 闸门缺失 → cc-apply 阻断。
