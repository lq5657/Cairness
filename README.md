...
第三阶段的路由指标只写入本地 `.cairness/observability/runtime-events.jsonl`：保存选择比例、回退率和未知源计数，不保存测试路径或源码内容，也不会自动修改或提交 `tests/test-policy.yaml`。`ci/optimize` 在能获得 Git diff 且 pytest 失败摘要可解析时，还会记录 normal 影子选择是否出现 routing escape；干净的 CI checkout 可通过 `CC_VERIFY_BASE_REF=<base-sha>` 提供比较基准。信息不足时保持 `null`，不把未知当成通过。`cc-optimize --json` 会在样本达到门槛后给出观察或校准建议；正式策略调整仍需经过完整测试和版本化变更。
