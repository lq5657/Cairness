...
```bash
cc-help
cc-start                         # 不确定阶段时，先查看状态和推荐下一步
cc-start --intent change         # 明确想进入 change 生命周期时
```

`cc-help` 是命令速查入口；`cc-help --advanced` 显示完整底层命令。
`cc-start` 的 `--intent` 表示目标意图，不表示当前阶段；省略 `--intent`（或使用
`--intent status`）会根据 `.cairness/changes/*/spec.md` 的持久化状态推荐下一命令。
...
测试路由会把脱敏运行摘要写入本地 `.cairness/observability/runtime-events.jsonl`，用于评估选择比例、全量回退率、未知源比例和 routing escape。该文件已被 Git 忽略，不包含测试路径或源码内容；设置 `DO_NOT_TRACK=1` 可关闭写入。

`ci/optimize` 会用全量 pytest 结果校准 normal 模式的影子选择。干净的 CI checkout 可通过 `CC_VERIFY_BASE_REF=<base-sha>` 提供变更基准；证据不足时结论保持 `null`，不会把未知当成通过。`cc-optimize --json` 只读取指标并给出建议，不会自动修改或提交 `tests/test-policy.yaml`；策略调整仍需经过完整测试和版本化变更。
...
| `cc-start` / `cc-start --intent status` | 不确定阶段时读取项目状态并推荐下一命令，不自动执行 |
| `cc-start --intent change` | 根据目标意图解释推荐入口；发现 `apply` 中断 change 时推荐 `cc-apply <change-id>` 恢复，不自动执行 |
