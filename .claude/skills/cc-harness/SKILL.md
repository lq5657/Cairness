...
- `.cc/` 只记录使用本 Harness 的目标项目状态；维护 `cc_spec` Harness 自身时，不得把 Harness 维护事实、路线图、任务状态或架构说明写入 `.cc/context/*`、`.cc/changes/*`、`.cc/audits/*` 或 `.cc/knowledge/*`。
- 在命令执行前通过协议校验输入和路径角色。
- 将 `.claude/runtime/commands/<command>.yaml` 视为已迁移命令的最高优先级运行时来源。
- 将 `.claude/workflows/cc-workflow.yaml` 视为状态、写权限和自动校验的脚本与 CI 真相源。
- 将 `.cc/context/domain-language.md` 视为目标项目共享业务词汇表。仅在需要时按 bounded context 拆分，不按编程语言拆分；不要把 Harness 框架术语表写入其中。
