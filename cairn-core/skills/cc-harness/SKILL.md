...
- Loop 中需要等待外部输入或资源时，调用 `cc-loop-step pause --session-id <id>`，恢复后调用 `cc-loop-step resume --session-id <id>`；不要把等待时间算入 active time。实际执行 wave 后，将每波 `wave_id/started_at/ended_at/planned_parallelism/actual_parallelism/wait_ms` 通过 `cc-wave-plan --execution-summary '<json>'` 上报。
