...
典型症状：
- 期望 `cc-init` 自动创建 `.claude/rules/`、`.claude/knowledge/`、`.claude/changes/templates/`
...
- 只把 `cc-init` 用于更新 `context/project-context.md`
...
- 产物落在仓库根目录 `changes/`、`audits/`，而不是 `.claude/` 下
- README、CLAUDE、rules 中的路径写法不一致

根因：
- 没把“本框架所有相对路径默认相对于 `.claude/`”讲清楚
- 文档间路径口径不统一，导致模型自行猜测

正确做法：
- 在总纲中明确 `.claude/` 是 harness 根目录
- 接入前先用预检清单核对路径解释一致性
...
- 不要在启动阶段全量读 `rules/`、`knowledge/`、业务代码
...
根因：
- 没有把“发现问题并修正规则”转化为 `knowledge/` 资产
- 只改模板，不补索引、清单和长期约定
