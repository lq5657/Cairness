...
| 5 | 生命周期状态转换可执行化 | 部分 | 写入侧 SSOT + 一致性检测已落地:`cc-event-write` 脚本(envelope writer,内存校验后追加,reuse `harness_runtime.events.validate_event` 形态保证,event_id 幂等 E_EVENTW001 拒写补强 E_EVENT015,dry-run,缺目录 E_EVENTW002 拒绝不 scaffold);`harness_runtime/events.py` 抽取 `validate_event`+常量(cc-event-check/cc-event-write 共享单一校验源,行为基线测试钉死);`cc-event-check` 新增 E_EVENT020(event↔spec 一致性:latest 非 unchanged transition.to 与 spec.md status 按兼容矩阵比对,opt-in by 文件存在,框架仓库/历史 change discover 返回 [] 天然豁免,经 cc-verify 两路接入);core.yaml 注册 event-write + runtime-core.schema 加 event-write 属性;模板占位行改空文件 + `events.example.jsonl` 示范。遥测消费者(cc-stats/cc-gate-stats/cc-budget-check)就绪。**待办**:14 命令 prose fanout(把 cc-event-write 调用接入 6 个真生命周期命令 manifest,8 个 none→unchanged 无需)+ E_EVENT021 链连续性 + backfill 工具。守护测试 26 例(baseline 2 + writer 13 + E_EVENT020 11) |
