...
| `cc-review` | 审查 change；保持 `review` | `change-id` | `.cc/changes/<change-id>/review.md`、`.cc/changes/task-board.md` 审查状态 | `review.md`、`.cc/changes/task-board.md`，必要时补充 `log.md` 中审查中断上下文 | task coverage、验证证据、Finding 状态、证据类型矩阵、角色契约、Finding 接受处置交互 | 直接改代码；直接归档；有 open Critical/Important 仍 pass；删除审计记录；未让用户选择就把 Finding 标为 `accepted` |
