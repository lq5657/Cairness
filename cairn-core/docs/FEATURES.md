...
29 个 topic rules 覆盖数据库变更、API 兼容、并发、性能、安全、配置、观测、发布等专题领域。触发方式分两组，互补运行：
...
- `.claude/`（=`cairn-core/` 的拷贝）：可升级的框架资产
- `.cairness/`：项目状态目录（context、changes、audits、knowledge）

`cc-cairn init` 从系统安装位置拷贝框架。`cc-cairn update` 拉取最新版并替换 `.claude/`，**不触碰 `.cairness/` 下的任何项目数据**。
