...
把 `.cc/audits/<audit-id>/report.md` 中选中的 Findings，整理成适合进入 `cc-propose` 的桥接材料。
...
- `.cc/changes/task-board.md` 的候选摘要
...
- 可写文件：仅 `.cc/audits/<audit-id>/to-change.md`、`.cc/changes/task-board.md`
- 必须校验：选中的 Findings、边界收敛、拆分粒度、验证等级提示、后续 `cc-propose` 可用性，且 task-board 不替代正式 spec；当选择或拆分有歧义时，必须获得用户对 Findings / scope 的显式选择
- 禁止行为：机械复制整份 audit、直接写 `.cc/changes/<change-id>/spec.md`、合并不相干问题、自动进入 `cc-propose`、默认全选 Findings、范围选择缺失仍写桥接文档
...
- 必须把推荐的 `change-id`、范围摘要、阻塞/依赖和下一步写入 `.cc/changes/task-board.md` 的 Backlog 候选，不得直接创建正式 change 文档
