...
| 语言 | Runtime Profile | Technology Catalog | 决策组 | 状态 |
|------|-----------------|--------------------|--------|----|
| Go | `.claude/runtime/languages/golang.yaml` | `.claude/runtime/technology/golang.yaml` | 25 | 可用 |
| Python | `.claude/runtime/languages/python.yaml` | `.claude/runtime/technology/python.yaml` | 16 | 可用 |
| TypeScript | `.claude/runtime/languages/typescript.yaml` | `.claude/runtime/technology/typescript.yaml` | 12 | 可用 |
| Java | `.claude/runtime/languages/java.yaml` | `.claude/runtime/technology/java.yaml` | 10 | 可用 |
| C++ | `.claude/runtime/languages/cpp.yaml` | `.claude/runtime/technology/cpp.yaml` | 8 | 可用 |

新语言应作为 runtime language profile 和 technology catalog 添加，而非新建顶层语言目录。前端语言（如 TypeScript）的 decision groups 支持 `cascading_effects` 声明框架选择对下游选项的约束。
