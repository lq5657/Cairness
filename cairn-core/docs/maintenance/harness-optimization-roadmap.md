...
| 2 | Agent 命令协议 | 部分 | 协议骨架在位。**声明一致性已闭环**:`protocol.yaml.input_contracts` 回填 9 槽(requirement_description/topic_description/project_idea/fix_description/research_depth/discuss_mode/continue_discussion/mode/scope),正向回填不反向改 commands(避免 readset 条件 key + workflow 重生扇出);cc-schema-check 新增 E_SCHEMA133(inputs 名未登记)/E_SCHEMA134(required 输入用 none 哨兵)/E_SCHEMA199(enum 槽缺 values),经 _PROTOCOL_CACHE 复用 merged protocol。**错误分类映射已闭环**:放宽 command-protocol.schema errorContract 加 `error_codes` optional 字段,protocol.yaml error_taxonomy 9 项挂现有 E_* 码(N:1 归类),E_SCHEMA131 守护字段形态。**events 错误码承载已落地**:command-event.schema 加 v2 可选 `error_codes` 字段(array of `^E_[A-Z]+[0-9]+$`,unique,镜像 errorContract 先例);cc-event-write 加 `--error-codes`(repeatable)+ E_EVENTW005 形态守护;harness_runtime/events.py 加 E_EVENT021(checker 校验,门控 `is not None` 基线 fixture 零漂移);与 verification_status 独立(passed 可带码/failed 可不带,不硬耦合)。**不做(用户决策)**:运行时 argv 值校验(E_INPUT* 严格契约检查)——契约当前够用,agent 从 spec 读 change_id 本就合规,格式校验属过度工程;missing_required_input/invalid_input 的 error_codes 保持空(那俩是 argv 校验的收据,跳过)。澄清:protocol.yaml 顶层缺 technology_decisions/language_profile 是分层资产设计(load_protocol_assets 合并),非漂移 |
...
| 5 | 生命周期状态转换可执行化 | 部分 | 写入侧 SSOT + 一致性检测 + 命令接入已落地:`cc-event-write` 脚本(envelope writer,内存校验后追加,reuse `harness_runtime.events.validate_event` 形态保证,event_id 幂等 E_EVENTW001 拒写补强 E_EVENT015,dry-run,缺目录 E_EVENTW002 拒绝不 scaffold);`harness_runtime/events.py` 抽取 `validate_event`+常量(cc-event-check/cc-event-write 共享单一校验源,行为基线测试钉死);`cc-event-check` 新增 E_EVENT020(event↔spec 一致性:latest 非 unchanged transition.to 与 spec.md status 按兼容矩阵比对,opt-in by 文件存在,框架仓库/历史 change discover 返回 [] 天然豁免,经 cc-verify 两路接入);core.yaml 注册 event-write + runtime-core.schema 加 event-write 属性;模板占位行改空文件 + `events.example.jsonl` 示范。**命令接入**:6 个生命周期命令 manifest(propose/apply/review/fix/test/archive)`steps:` 末尾加 `record_state_transition_event_via_cc_event_write`(自描述 step id,--from/--to 取自 manifest state 块;8 个 none→unchanged 命令不接,无状态审计价值),SKILL.md:81 从"手写 events.jsonl"收敛为"调用 cc-event-write"。遥测消费者(cc-stats/cc-gate-stats/cc-budget-check)就绪。**待办**:E_EVENT021 链连续性 + backfill 工具 + "该有事件却没"硬执行检测(hook/verify 层,opt-in 政策下非阻塞)。守护测试 26 例(baseline 2 + writer 13 + E_EVENT020 11);233 passed |
...
| 9 | 语言 profile 分离 | 部分 | 5 语言 profile(cpp/golang/java/python/typescript)数据驱动平等已就位:各有 language yaml + technology catalog + fixture;cc-apply/cc-review/cc-fix conditional topic_rules 平等列 10 trigger key;dispatcher run_project_checks 完全数据驱动无语言分支。显示/诊断层 Go 偏见已清(capability_display_name 泛化为取命令前两段,diagnosis_for Go 特判分支移除走 default)。保留:GOCACHE 注入(Go 测试功能必需,非偏见)、core.yaml default=golang(本仓 Go 主导)、go_validation legacy(向后兼容)。**待办**:C++ profile 一致性(detection module_file=CMakeLists.txt vs fixture Makefile vs verification make 三不一致 + cpp fixture 解析为 confirmation_required,单独立项 D6)+ profile 强制执行对称化(cc-init/cc-enrich-context 对齐 cc-new-project 闸门)+ 4 语言 eval case 补齐 |
...
| B2 | 语言 profile 显示/诊断层去 Go 偏见 | #9 | `cc-verify capability_display_name` 泛化为取命令前两段(不再特判 go test/vet/golangci-lint,Go 行为不变,其他语言升级);`diagnosis_for` Go 特判分支移除走 default。保留 GOCACHE(功能必需)/core.yaml default(本仓现状)/go_validation legacy(向后兼容)。无新测试(纯重构,现有 207 测试守护) | ✅ 完成 |
| D6 | C++ profile 一致性修复 | #9 | 待办:detection `module_file: CMakeLists.txt` 但 cpp-library fixture 是 Makefile 项目;verification 写死 `make test`/`make build`;cpp fixture 解析为 confirmation_required(detection 未命中)。三不一致 + 解析失败,牵动 detection confidence/fixture 适配/resolve 逻辑,单独立项 | 待办 |
