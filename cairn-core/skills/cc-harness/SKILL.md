...
- 生命周期命令执行状态转换时，调用 `.claude/scripts/cc-state-transition` 一次性原子完成 spec.md `status:` 更新与 `events.jsonl` 事件追加（传入 `--change-id`/`--command`/`--from`/`--to`/`--summary`/`--evidence`，`--from`/`--to` 取自该命令 manifest 的 `state` 块）；不要手改 spec.md 的 `status:` 字段，也不要手写 `events.jsonl`。该脚本先校验当前 spec status 等于 `--from`（不符则 E_STATE001 硬失败，指向 cc-reconcile），再按「事件先于 spec」的顺序写入，使唯一的崩溃窗口只可能是 event↔spec 可检测方向（cc-event-check E_EVENT020），不产生静默漂移。`--from none`（创建，spec 由 cc-propose 写）与 `--to unchanged`（cc-review/fix/test 审计 no-op）只写事件不改 spec。底层事件追加仍复用 `cc-event-write` 作为 events.jsonl 单一写入源。
