...
升级后 Codex 项目只使用项目级 `.agents/skills/cc-harness/SKILL.md`。生命周期
命令（如 `cc-apply <change-id>`）由 Codex agent 按 runtime contract 直接执行，
不对应物理脚本；确定性运行时工具和只读入口才从 `.codex/scripts/<command>`
调用，不应从 `.claude/scripts/*` 调用。`cc-doctor-check`、`cc-lint` 会按当前 adapter 检查，发现重复活动 Skill
...
- `cairn-core/scripts/harness_runtime/wave_execution.py` — adapter 无关的并发派发、expected-task ledger、join/cleanup 与实际并行度计算
- 执行期产物(目标项目): `.cairness/changes/<id>/wave-plan.json`、`.cairness/changes/<id>/waves/wave-N.md`

### 迁移与回滚

- 既有 change 可暂时使用完整、机器可读的 `依赖 / Wave` 行；含自然语言依赖、缺失字段或正文/Task 图不一致的声明会被 `E_WAVE006` 阻塞。新 change 必须在 `tasks.md` frontmatter 使用 `task_graph` 作为调度单一真相源。
...
- `cc-apply` 保持单 Wave 进行中规则。同一 Wave 内只有依赖已满足、`parallel_safe: true` 且写集合经验证互斥的正式 task 可并行；所有 worker 必须 join 并完成 cleanup 后才能推进下一 Wave。
