...
|------|----------------|----------|----------|
| 2026-04-11 15:20 | repo 创建路径缺少显式 timeout | 二次审查发现 repo 写入路径无最小超时边界 | `docs/examples/changes/user-create-api-fix/review.md` |
| 2026-04-11 15:20 | service 返回错误缺少上下文包装 | 二次审查发现 `Create` 路径直接暴露底层错误 | `docs/examples/changes/user-create-api-fix/review.md` |
...
| 2026-04-11 15:55 | `[user-create-api-fix] 更新 review 并补回归测试` | `internal/service/user_service_test.go`, `docs/examples/changes/user-create-api-fix/review.md` | `L2` | 补齐 fix 所需回归证据并回写 findings 状态 |
...
|--------|------------|------|----------|----------|----------|
| `fix-must-update-review` | `cc-fix` 不只是改代码，还必须同步回写 `review.md` Findings 状态 | `docs/examples/changes/user-create-api-fix/review.md` | `knowledge/index.md` | 更新既有知识 | 已沉淀 |
| `finding-root-cause-split` | 修 fix 前应先区分症状、失败点和根因，再形成最小修复假设 | `docs/examples/changes/user-create-api-fix/log.md` | `knowledge/index.md` | 新增知识 | 已沉淀 |
