...
| Minor | 架构边界 | 当前样例强调 `handler -> service -> repo`，但 `project-context.md` 里尚未把这个真实分层写成明确事实 | `.cc/context/project-context.md:L1` | 在 `cc-init` 或 `cc-inspect-codebase` 后补充实际调用关系说明 | open |
...
- 是否建议立即创建 `.cc/changes/<change-id>/`：是，建议 `user-domain-boundary-guard`
- 是否建议补 `project-context.md`：是
- 是否建议沉淀到 `.cc/knowledge/`：是，建议沉淀“Handler 只做协议适配，业务规则留在 Service”
