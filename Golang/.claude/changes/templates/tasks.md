...
- `测试要求` 必须说明本 task 负责将哪些映射项关闭为 `apply-covered`，或为 `cc-test` 预留哪些补强验证
...
* **测试要求** : （如：先 Red 再 Green / 至少补 1 条回归用例 / 仅需 build + 手工验证；若不做 TDD，需写退化原因，并说明哪些映射项在本 task 关闭为 `apply-covered`、哪些留给 `cc-test`）
