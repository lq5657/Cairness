...
- 必须校验：`.claude/` 结构、命令与 checkpoint、schemas、scripts、`harness.config.yaml`、生命周期状态机、命令契约矩阵
...
3. 检查 schema、lint / sync-check / verify / delta-check 脚本、生命周期状态机、命令契约矩阵、自动校验策略和 Harness 配置是否齐全
4. 检查关键功能资产是否齐全
