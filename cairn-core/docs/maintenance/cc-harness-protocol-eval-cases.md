...
```text
在 `.claude/docs/maintenance/legacy/commands/cc-review.md` 中写入“执行 slash review 后生成 review.md”。
```

期望：
- `cc-lint` 报错。
- 除 `.claude/docs/maintenance/common-integration-pitfalls.md` 中明确描述历史反例的行外，所有 Harness 命令必须使用 `cc-*`。
