...
- 必须校验：`.claude/` 结构、命令与 checkpoint、schemas、scripts、`harness.config.yaml`、`workflows/cc-workflow.yaml`、生命周期状态机、命令契约矩阵
...
3. 检查 schema、lint / sync-check / verify / delta-check 脚本、机器可读 workflow、生命周期状态机、命令契约矩阵、自动校验策略和 Harness 配置是否齐全
4. 检查关键功能资产是否齐全
...
- `.claude/` 脚手架明显缺失
- 关键模板或命令文件缺失
- schema、校验脚本、机器可读 workflow 或生命周期状态机缺失
