...
9. 提问澄清：逐个提出高价值问题并等待回答，只问会影响 spec 正确性、范围边界、任务拆分的事项；不得只把“待澄清”写进文档或摘要后继续
...
- 若存在会影响 scope、验收标准、风险判断或 task 拆分的未决问题，本轮结果必须把状态标为 `blocked` 或 `partial`，并直接向用户提出编号问题。
- 澄清问题必须带有期望回答形态；当可枚举时给出明确选项，例如“选 A / B / C，或补充你的约束”。
- 只在 `spec.md` 的“待澄清”章节列出问题不算完成澄清交互；最终输出也必须请求用户回答这些问题。
- 存在阻塞性澄清项时，不得把 HARD-GATE 表述为已就绪，也不得把下一步写成直接执行 `cc-apply`。
- 若已生成 `spec.md` / `tasks.md` 且自动校验通过，并且没有阻塞性澄清项，最终输出必须展示 HARD-GATE 确认项：
  - 当前 spec / tasks 版本或 diff 摘要
  - 本次 scope 和本次不做
  - 验收标准和验证映射
  - 已接受风险与是否需要人工审查
- HARD-GATE 输出必须要求用户选择：
  1. 确认 scope、风险和验证映射，允许下一步执行 `cc-apply`
  2. 要求修改 spec / tasks
  3. 阻塞，先补充澄清
- 只有用户明确选择确认后，才允许在 `spec.md` 的 HARD-GATE 记录中写入确认字段；用户未选择时只能保持等待确认。
...
- `docs/maintenance/legacy/checkpoints/cc-propose.md`
