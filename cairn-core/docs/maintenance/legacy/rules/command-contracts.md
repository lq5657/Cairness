...
- context / audit / explain 类命令不得改变 `.cairness/changes/<change-id>/spec.md` 的生命周期状态。
...
- 写入 `.cairness/context/dev-map.md`、`.cairness/changes/task-board.md` 或 `.cairness/knowledge/*` 时，必须遵守 `rules/memory-policy.md`；调用 reviewer 或子角色时，必须遵守 `docs/maintenance/legacy/rules/role-contracts.md`。
