...
| 项目 | 内容 |
|------|------|
| 原始诉求 | 增加一个“创建用户”的标准接口，替代调用方绕过服务层直接写库 |
| 关键澄清 | email 是否作为幂等键；重复 email 返回业务错误还是直接透传底层错误；本次是否引入真实集成环境 |
| 收敛后的目标 | 在既有 `handler -> service -> repo` 分层中补齐创建链路，保证幂等、错误语义和最小回归验证 |
| 收敛方式 | light-clarify |
