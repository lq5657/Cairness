...
Codex lifecycle commands are agent workflows: invoke them literally (for
example, `cc-apply <change-id>`) and let the `$cc-harness` skill execute the
runtime command contract. They are listed under `migrated_commands` and do
not have a physical `.codex/scripts/<command>` entrypoint.

Only deterministic runtime scripts and `readonly_entrypoints` are executed
through `.codex/scripts/<command>`. Never substitute a `.claude/scripts/*`
path, even when a Claude Code adapter is installed in the same project. In
particular, the stage probe is:
...
literal shell path to execute. When a manifest names a deterministic script
for Codex, use the physical `.codex/scripts/...` path.
