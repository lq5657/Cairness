...
|------|----------------|----------|--------------|-----------|
| Service 错误包装保留底层 error | L2 | build | 展示错误包装回归测试，确认 `%w` 保留原始错误 | Task 1 / Task 2 |
| Repo 调用具备最小超时边界 | L2 | package | 展示 timeout 接入代码证据与补充测试/审查说明 | Task 1 / Task 2 |
| Findings 修复链路完整保留 | L1 | manual | review 文档回写 `fixed`，不删除原 Findings | Task 2 |
