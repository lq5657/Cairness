...
- Subagent 角色必须在 `.claude/docs/maintenance/legacy/rules/role-contracts.md` 中注册。
...
主流程负责去重 finding、设定严重等级，并写入 `.cairness/audits/<audit-id>/report.md`。
...
主流程必须遵守单 Wave 进行中规则。同一 Wave 内，只有 `task_graph`
声明 `parallel_safe: true`、依赖已满足且经 `cc-wave-plan` 验证写集合互斥的
正式 task 才能并行；其余 task 必须串行。主流程必须为 Wave 建立预期
subagent 账本，并在所有 task 进入明确终态且 orphan cleanup 完成后才能
进入下一 Wave 或结束 `cc-apply`。
...
- subagent 角色已在 `.claude/docs/maintenance/legacy/rules/role-contracts.md` 中注册
