...
- 所有 `cc-*` 命令都必须对照 `workflows/cc-workflow.yaml`、`rules/command-contracts.md` 与 `rules/role-contracts.md` 的状态机角色、执行角色、输入输出、可写文件、校验项和禁止行为
...
- `cc-init` 只负责识别可长期复用的基础项目事实，不负责安装框架

### `cc-enrich-context`

用途：
- 在 `cc-init` 基础上补充更完整的项目事实画像
- 补充分层、调用关系、日志、配置、测试、可观测性、依赖边界和高风险点
...
- 每个 workflow 命令都必须声明 `roles`，且角色必须先在 `rules/role-contracts.md` 登记
