...
- 路径解释是否一致
- 文本命令是否会和宿主命令系统冲突
- `/init` 是否会跑偏成“安装脚手架”或“乱建目录”
- 当前项目是否适合进入下一步 `/inspect-codebase` 或 `cc-propose`

如果这份清单没通过，不建议直接开始正式使用。

#### 1. 脚手架完整性

目标：确认目标项目已经安装好 `.claude/` 脚手架，而不是指望 `/init` 临时补齐。
...
| 示例可选但清晰 | 是否包含 examples 已明确，不会被误当成 `/init` 产物 | [ ] |
...
| `/init` 不跑偏 | 只识别事实，不安装脚手架 | [ ] |
| `inspect-codebase` 可落产物 | 能输出带证据的 audit 报告 | [ ] |
| 维护者理解边界 | 知道什么时候该 `/init`，什么时候该先装脚手架 | [ ] |
