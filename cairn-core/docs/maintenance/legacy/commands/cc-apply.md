...
- `context-curator`：在模块边界或验证入口变化时更新 `.cairness/context/dev-map.md`。
...
- 输入：`change-id`
- 输出：代码改动、change 文档更新、必要的 `.cairness/context/dev-map.md` / `.cairness/changes/task-board.md` 更新、按配置生成 commit
- 可写文件：task 声明范围内代码，当前 change 的 `spec.md`、`tasks.md`、`log.md`，必要的 `test-spec.md`、`.cairness/context/dev-map.md`、`.cairness/changes/task-board.md`
...
- 首次进入实现前必须保存验证基线：`.claude/scripts/cc-verify --change <change-id> --json --output .cairness/changes/<change-id>/baseline/pre-apply.json`
...
- 开始某个 task 前，应读取 `.cairness/context/dev-map.md` 和 `.cairness/changes/task-board.md`，确认影响模块、既有验证入口、阻塞/依赖和下一命令记录没有冲突
- 若项目存在 `.cairness/context/mvp-roadmap.md`，开始当前 task 前还应确认该 task 与本次 change 在 roadmap 中的定位未发生偏移
...
- 若实现导致模块边界、关键链路、测试入口或易错边界发生实质变化，必须同步 `.cairness/context/dev-map.md`；每个 task 完成或阻塞时必须同步 `.cairness/changes/task-board.md`
...
- 若 `validation.auto_run = true`，当前 task 文档同步后必须运行 `.claude/scripts/cc-verify --change <change-id> --json --output .cairness/changes/<change-id>/baseline/post-task-<n>.json`
- 当前 task 完成前必须运行 `.claude/scripts/cc-delta-check --before .cairness/changes/<change-id>/baseline/pre-apply.json --after .cairness/changes/<change-id>/baseline/post-task-<n>.json`；若出现 `new-failure`，当前 task 不得标记为 `done`
- 所有 task 完成后，将 `spec.md` 状态改为 `review`
- 切换到 `review` 前必须再次运行 `.claude/scripts/cc-verify --change <change-id>`；失败时保持 `apply` 并记录阻塞原因
...
11. `.cairness/changes/task-board.md` 已同步当前状态、阻塞/依赖和下一命令；如模块导航变化，`.cairness/context/dev-map.md` 已同步
