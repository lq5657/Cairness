...
4. **代码现状必须有出处**  — 每个结论必须标注文件路径和结构体/函数名，不接受"我认为"、"通常来说"
...
前置检查 spec + tasks + 用户确认 。 逐 task 执行，每个 task 完成后展示验证证据（Verification 铁律，如 `go build` 或 `go test`） 。 零偏差原则：Plan 是合同，AI 是打印机。 自动 git commit（一个 task 一个 commit）。
