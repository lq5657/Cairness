# cc-enrich-context

## 用途

`cc-enrich-context` 用于在已有 `project-context.md` 基础上补充更完整的项目事实画像，并更新 `.cairness/context/dev-map.md` 的开发导航。
它承接 `cc-init` 之后尚未确认、但后续命令会受益的高解释成本事实。

它的目标不是做项目体检或输出 Findings，而是在“基础事实层”之上补齐“补充事实层”，例如：
- 实际分层与调用关系
- 日志方案与关键字段
- 配置管理策略
- 可观测性现状
- 测试分层与验证方式
- 领域特性与高风险点

它不是：
- 脚手架安装命令
- 存量问题审查命令
- 系统讲解命令
- 变更提案命令

## 触发场景

适用于：
- `cc-init` 已完成，但 `project-context.md` 仍有较多“待确认”
- 需要为后续 `cc-inspect-codebase`、`cc-propose` 提供更强的项目事实认知
- 需要补充分层、日志、配置、测试、可观测性等高解释成本上下文

不适用于：
- `.claude/` 脚手架尚未安装完整
- `project-context.md` 的基础事实层尚未建立
- 想直接开始代码问题审查
- 想输出讲解型系统说明
- 想直接开始实现需求

## 输入

命令格式：
- `cc-enrich-context`

展示 checkpoint 表时：
- 必须把状态写入 `结果` 列
- `结果` 仅允许填写 `✅`、`❌`、`⚠️` 或 `N/A`
- 不要在 `检查项` 列使用 `[ ]` / `[x]` 代替结果

## 输出

产出：
- 增量更新 `.cairness/context/project-context.md` 中的“补充事实层”内容
- 增量更新 `.cairness/context/dev-map.md` 中的模块边界、关键链路、验证入口和易错边界

不产出：
- `.cairness/changes/<change-id>/`
- `.cairness/audits/<audit-id>/`
- `.cairness/context/system-overview.md`
- 业务代码修改

## 命令契约

以 `docs/maintenance/legacy/rules/command-contracts.md` 中 `cc-enrich-context` 行为准：
- 状态机定位：项目事实补充命令，不创建也不改变 change 状态
- 输入：无
- 输出：`.cairness/context/project-context.md` 的补充事实层、`.cairness/context/dev-map.md` 的开发导航
- 可写文件：仅 `.cairness/context/project-context.md`、`.cairness/context/dev-map.md`
- 必须校验：基础事实层已存在、关键假设已确认或降级为待确认、证据位置和信心等级清楚，且长期记忆写入符合 `rules/memory-policy.md`
- 禁止行为：输出 Findings、创建 change 或 audit、把假设写成事实、自动转入 `cc-inspect-codebase` / `cc-propose` / `cc-apply`

## 必守边界

- 只补充项目事实，不创建脚手架资产
- 只回写 `.cairness/context/project-context.md` 和 `.cairness/context/dev-map.md`
- 不得把 `cc-enrich-context` 扩展为 `cc-inspect-codebase`、`cc-review` 或 `cc-explain-system`
- 允许读取更多入口和样本代码，但仍以“确认事实”为目标，不输出问题审查结论
- 允许提出“基于证据的待确认假设”，但不得把假设直接写成事实
- 假设确认只服务于事实补充，不得扩展为方案比较、问题审查或 change 提案
- 若证据不足，必须保留“待确认”，不得把推测写成事实

## 补充目标

`cc-enrich-context` 重点补充以下高解释成本事实：
- 关键链路索引
- 外部依赖与集成边界
- 已知脆弱区域的事实级描述
- 实际分层与调用关系
- 日志方案与关键字段
- 配置管理策略
- 可观测性现状
- 测试分层与验证方式
- 领域特性与高风险点

其中优先级默认是：
1. 关键链路索引
2. 外部依赖与集成边界
3. 已知脆弱区域

若时间或证据预算有限，应先把这三类补稳，而不是平均铺开所有章节。

## 与 `cc-init` 的分工

- `cc-init` 负责沉淀“基础事实层”，解决“从哪里开始看”
- `cc-enrich-context` 负责沉淀“补充事实层”，解决“系统大致如何组织”

`cc-enrich-context` 可以更深入，但仍应坚持“事实补图”而不是“问题审查”。
其中“已知脆弱区域”只用于记录后续命令需要注意的事实边界，不等同于 Findings 或整改结论。

## 允许读取的范围

允许读取：
- `.cairness/context/project-context.md`
- 根目录和关键子目录结构
- 多个启动、配置、日志、依赖注入入口文件
- 关键测试样本
- 为确认分层或关键链路所必需的少量调用链代码

禁止读取：
- 以问题挖掘为目标的全仓审查式扫描
- 与上下文补充无关的大量代码区域
- 把“补充上下文”写成 Findings、审查结论或 change 草稿

## 默认执行流程

1. 确认 `.claude/` 脚手架与 `.cairness/context/project-context.md` 存在
2. 读取当前 `project-context.md`，识别其中的“待确认事项”和薄弱区域
3. 优先围绕关键链路、外部依赖边界、脆弱区域做定向补充读取；再按需补充分层、日志、配置、测试、可观测性
4. 形成“待确认假设清单”，注明主要证据位置、信心等级与潜在影响
5. 与用户确认或纠偏关键假设
   - 默认必须停下来确认会影响事实回写准确性的关键假设
   - 每轮最多发起 1 组阻塞式确认，且阻塞假设不超过 3 条
   - 超过 3 条时，优先保留最影响事实准确性的 1-3 条，其余写入“待确认事项”
   - 不影响当前事实回写的非阻塞假设，直接写入“待确认事项”，不强制打断
6. 只将已确认事实回写 `project-context.md` 的补充事实层和 `dev-map.md` 的开发导航
   - `关键链路索引` 默认最多保留 3-5 条高价值链路，并标注 `P0 / P1 / P2`
7. 对仍无法可靠确认的内容继续保留“待确认”
8. 结束，不自动进入 `cc-inspect-codebase`、`cc-propose`、`cc-explain-system` 或其他命令

## 交互输出要求

- 阻塞式确认必须直接向用户提出编号问题，并等待回答；不得只把“待确认假设清单”写进文档或摘要。
- 每个阻塞假设必须说明：证据位置、信心等级、若写错会影响哪些后续命令或事实回写。
- 可枚举时必须给出明确选择，例如“确认 / 修正为…… / 保留待确认，不写入事实层”。
- 用户未回答前，只允许把该项保留为“待确认事项”；不得写入 `project-context.md` 的已确认事实层或 `dev-map.md` 的确定导航。
- 若最终输出中仍有阻塞假设，下一步必须是回答关键假设确认问题，而不是继续进入审查、提案或实现命令。

## 失败处理

若以下情况出现，必须停止并说明：
- `.claude/` 脚手架缺失
- `.cairness/context/project-context.md` 不存在
- 基础事实层明显缺失，无法可靠开展补充
- 项目结构不足以支撑上下文补充
- 关键链路证据不足，无法可靠判断
- 关键假设尚未获得用户确认，且该假设会影响事实回写准确性

恢复要求：
- 若仅部分关键假设已获确认，只允许回写对应已确认事实；未确认部分继续保留“待确认”
- 若待确认关键假设超过 3 条，必须先收敛为最关键的 1-3 条再发起阻塞确认
- 恢复执行时，必须从“关键假设确认”步骤继续，而不是重做整轮上下文补充

## 执行后建议

执行完成后，下一步通常是按目标选择其一：
- 若需要做存量项目体检：执行 `cc-inspect-codebase <mode>`
- 若已有明确需求：执行 `cc-propose <需求描述>`
- 若需要输出面向维护者的讲解材料：执行 `cc-explain-system`

## 需要加载的附加文件

- `checkpoints/cc-enrich-context.md`
- `.cairness/context/project-context.md`
- `.cairness/context/dev-map.md`
- `rules/memory-policy.md`
