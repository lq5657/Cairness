...
- 文件位置：`.claude/docs/examples/changes/user-create-api/spec.md`
...
增加一个“创建用户”的标准接口，替代调用方绕过服务层直接写库
→ 澄清 email 是否作为幂等键、重复 email 的错误语义、是否引入真实集成环境
→ 在既有 `handler -> service -> repo` 分层中补齐创建链路，保证幂等、错误语义和最小回归验证
（收敛方式：`light-clarify`）
