...
|------|----------------|----------|--------------|-----------|
| 创建用户成功主链路 | L3 | chain | 展示 `Create` 成功路径回归测试，并说明 Handler 到 Service 的接线关系 | Task 1 / Task 2 |
| email 重复不得重复创建 | L2 | chain | 展示重复 email 回归测试，确认返回稳定业务错误 | Task 1 / Task 2 |
| 新增接口不破坏旧调用方 | L2 | manual | review 对照新增接口语义与兼容性分类，确认属于 `compatible_addition` | Task 2 |
