...
- `/propose` 必须为每个 change 声明最低验证等级和证据要求。
- `/apply` 必须实际执行并闭合当前 task 承接的最低验证要求，不能只写文档状态。
...
- `cc-test` 只能补更高等级证据、环境型验证或额外补强，不能替代本应在 `cc-apply` 中完成的最低验证。
...
- `/apply`：按 task 展示当前已完成的验证证据；不得跳过本次 change 声明的最低等级，也不得用 `go build ./...` 代替 `L2+` 证据。
